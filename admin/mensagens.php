...
// Lógica: Se clicarem no botão "Marcar Lida"
if (isset($_GET['marcar_lida'])) {
    $id = intval($_GET['marcar_lida']);
    // Atualiza o estado na BD (lida = 1)
    $stmt = $dbh->prepare("UPDATE mensagens SET lida = 1 WHERE id = ?");
    $stmt->execute([$id]);
    header("Location: mensagens.php"); // Recarrega a página limpa
    exit;
}

// Buscar todas as mensagens
// Ordenação: As não lidas (0) aparecem primeiro, depois as mais recentes
